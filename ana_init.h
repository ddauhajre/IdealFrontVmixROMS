      ...
      real :: h0          = 60.0     ! m. boundary layer beyond filament or front
      real :: dh0         = 15.      ! m boundary layer amplification (bl = h0 + dh0)
      real :: L           = 2000     ! m  transition scale of hbls (NOT USED)
      real :: xfront      = 12800!6412.5   ! m position of front
      real :: mfront                 ! magnitude of frontal gradient (sets Ro)
...
!         h_sbl = h0 + dh0*exp(-(( xr(i,j) /L )**2))
          h_sbl = h0 + dh0*( tanh( mfront*(xr(i,j) + xfront)) - tanh( mfront*(xr(i,j) - xfront)))
